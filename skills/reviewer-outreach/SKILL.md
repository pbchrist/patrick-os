...
The governing hard gates live in `config/reviewer-outreach.json`. This skill never sends. It can only emit `SENDABLE` or `NOT SENDABLE` with evidence. A separately authorized connector action may act on a `SENDABLE` packet.
...
- The current daily external-send count is known before declaring `SENDABLE`.
...
10. **Specify the approved package.** Nyandor sender identity; real clickable BookFunnel link supplied at runtime; side-by-side covers attachment under the configured byte limit; Nyandor author signature.
...
12. **Check volume.** If the current daily count is already at `daily_send_cap`, output `NOT SENDABLE: DAILY CAP` unless a human explicitly changes the operating limit outside this skill.
13. **Emit the packet.** Include evidence, subject, body, required attachment, contact route, and exactly one of: `SENDABLE`, `FORM REQUIRED`, or `NOT SENDABLE`.
14. **Specify writeback.** The packet must name the CRM fields that the execution layer must update after a successful send. This skill does not claim the writeback occurred.
...
- The packet requires the side-by-side cover asset and enforces the configured size limit.
...
- The daily-volume gate passes before `SENDABLE` is emitted.
- The packet states exactly what must be written back after execution.

## Failure modes

- **Stale personalization.** Present-tense wording built from an old post. This creates the appearance of fabricated research. Find current evidence, reframe historically, or remove the line.
...
- **Oversized attachment.** A prior deliverability test flagged the original 1.86 MB PNG. Require the compressed asset under the configured limit.
...
- **Volume spike.** The packet says `SENDABLE` after the configured daily cap is already reached.
...
- The current daily count has reached the configured cap.
...
This wording is allowed only because live, dated evidence supports `still actively` at packet-generation time.
...
`fixtures/freshness-gate-present.json` asserts that the composed procedure contains the freshness gate, the evidence-ledger requirement, the daily cap, and the explicit no-send boundary.
