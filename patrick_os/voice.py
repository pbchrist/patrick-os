...
def voice_dir(base=None):
    return root(base) / "voice"


def scope_path(scope, base=None):
    """Map a scope string to the file that owns it."""
    directory = voice_dir(base)
    if scope == "global":
        return directory / "global.md"
    kind, _, name = scope.partition(":")
    if not name:
        raise VoiceError(f"scope {scope!r} needs a name, e.g. 'channel:linkedin'")
    if kind not in {"channel", "project", "skill"}:
        raise VoiceError(
            f"unknown scope kind {kind!r}; expected global, channel, project, or skill"
        )
...
def load_scope(scope, base=None):
    path = scope_path(scope, base)
    if not path.is_file():
        return []
...
def next_rule_id(scope, base=None):
    ...
    """
    prefix = SCOPE_PREFIXES[scope.partition(":")[0]]
    path = scope_path(scope, base)
    highest = 0
    if path.is_file():
...
def append_rule(scope, text, *, source=None, base=None):
    ...
    """
    path = scope_path(scope, base)
    rule_id = next_rule_id(scope, base)
